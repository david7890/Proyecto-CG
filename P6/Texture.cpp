...
*/
	...
	GL_LINEAR  aplica sampling y blending de texels más cercanos. GL_NEAREST aplica sample de texel más cercano
	*/
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);//Cuando se ve mas pequeño con MIN
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);//cuando se mas grande con MAg
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);//aqio mo se comoloca una optimizacion, por lo que nearest hace que las texturas se vean pexeliados 
	//GL linear a cada pixel se le asigna el dato mas peuqeño de la textura, por lo que se considera como una optimixaciones 
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, texData);
	glGenerateMipmap(GL_TEXTURE_2D);//quiero que me generaes autonomaticamente texturas que automaticamente en que este la cammara 
	//este a una cierta distancia, este se ajuste, es decir, si te alejas, la calidad de la textura baja. Si te acercas a la textura se mejorara
...
{	//UnitTexture
	...
	/* Le dice al shader que necesata algo para guarda la informacion de una textura que ya se la enviara
	*/
...
/*Dentro de la textura se menjara dos ejes, el eje T7v, que sera la y, por otro lado el eje S/u seria el eje X

clamo y clam en eje t y smostrara la textura en todo el cuadrado
clampo en T y repeat en S, nos mostrara la textura en dos repeticiones veticales }
en repeat en T y clamp en S, seran dos texturas en horizontal 
repeat en T y S sera 4 veces de la textura 
